...
class CompanyValidatorServiceImpl implements CompanyValidatorService {
    ...
    @Transactional
    @Override
    public void fcaNumberUnverified(String companyId) throws CompanyNotFoundException {
    ...
    public void fcaNumberVerified(final String companyId) {
        log.info("Verifying FCA number for company [ {} ]", companyId);
        try {
    ...
    public void billingIsActive(final String companyId) {
        log.info("Activating billing for company [ {} ]", companyId);
        try {
            CompanyValidatorBO companyValidatorBO = this.getOrCreateCompanyValidatorBO(companyId)
    ...
    @Override
    public void billingInactivated(String companyId) throws CompanyNotFoundException {
        CompanyValidatorBO brokerValidatorBO = this.getOrCreateCompanyValidatorBO(companyId)
    ...
    public boolean checkIfCompanyVerified(final String companyId) {
        return companyValidatorRepository.findByCompanyId(companyId)
                .map(company -> company.isFcaNumberVerified() && company.isFcaNumberVerified())
                .orElse(false);
    }

    @Override
    public Optional<CompanyValidatorResponse> findByCompanyId(String companyId) {
        return companyValidatorRepository.findByCompanyId(companyId)
