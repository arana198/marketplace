...
public class AddressFinderService {
    ...
    public Address getAddressByPostcode(final String postcode)
        ...
        log.debug("Getting address for location {}", postcode);
    ...
    private List<String> getHouseNumber(final String postcode) throws IOException, AddressNotFoundException {
        ...
        headers.add("Cookie", "_ga=GA1.2.542764837.1516538164; _gid=GA1.2.1903034621.1516538164; wow.anonymousId=01b837bc-bc00-4aea-8183-2c740cf2250d; wow.schedule=wowTracking_C; wow.session=01b837bc-bc00-4aea-8183-2c740cf2250d; wow.utmvalues=; gwcc=%7B%22fallback%22%3A%2201619242800%22%2C%22clabel%22%3A%22qpbCCIDYsm4Q_aKGzgM%22%2C%22backoff%22%3A86400%2C%22backoff_expires%22%3A1516624563%7D; _gat=1; freeaddressd=2015-01-01; freeaddressc=0");
    ...
    private Postcode getPostcodeDetails(final String postcode) throws PostcodeNotFoundException, OutcodeNotFoundException {
        try {
            ...
            if (ex.getRawStatusCode() == HttpStatus.NOT_FOUND.value()) {
                log.info("Could not find location {}", postcode);
    ...
    private Outcode getOutcodeDetails(final String outcode) throws OutcodeNotFoundException {
        try {
            ...
            if (ex.getRawStatusCode() == HttpStatus.NOT_FOUND.value()) {
                log.info("Could not find outcode {}", outcode);
